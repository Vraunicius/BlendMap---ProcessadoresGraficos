...
public class MeshFactory {
    ...
    private static float calcLinear(float min, float max,
    ...
    private static float calcPiramid(float min, float max,
                                     float value) {
    ...
    public static Mesh loadTerrain(File file, float scale, int texRepeat) throws IOException {
        ...
        BufferedImage Blend = new BufferedImage(width,depth, BufferedImage.TYPE_INT_ARGB);

        List<Vector2f> Lista = new ArrayList<>();
        tx = 1.0f / width;
        ty = 1.0f / depth;
        ...
        for (int z = 0; z < depth; z++) {
            for (int x = 0; x < width; x++) {
                ...
                Lista.add(new Vector2f(x*tx,z*ty));
                
                Color pixel = Normalize(calcLinear(0.75f, 1.00f, h, false),
                                    calcPiramid(0.50f, 0.80f, h),
                                    calcPiramid(0.15f, 0.60f, h),
                                    calcLinear(0.00f, 0.16f, h, true));
            Blend.setRGB(x,z,pixel.getRGB());
            }

        }

        ImageIO.write(Blend,"png",new File("/Users/vinil/Desktop/img/opengl/textures/BlendMap.png"));

        return new MeshBuilder()
                    .addVector3fAttribute("aPosition", positions)
                    .addVector3fAttribute("aNormal", normals)
                    .addVector2fAttribute("aTexCoord", texCoords)
                    .addVector2fAttribute("aBlendCoord", Lista)
                    .setIndexBuffer(indices)
                    .create();
    }
    static Color Normalize(float x,float y,float z, float w){
        return new Color((x > 1) ? 1 : (x < 0) ? 0 : x,
                        (y > 1) ? 1 : (y < 0) ? 0 : y,
                        (z > 1) ? 1 : (z < 0) ? 0 : z,
                        (w > 1) ? 1 : (w < 0) ? 0 : w);
